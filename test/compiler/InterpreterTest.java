...
public class InterpreterTest {
    ...
    public void test1() throws Exception, SyntaxErrorException {
        Compiler cmp = new Compiler(new BufferedReader(new FileReader(new File("/Users/deca/Desktop/repo_comp/funny/test/resources/test.funny"))));
        Expr expr=cmp.program();
        new InvokeExpr(expr, new ArrayList<>()).eval(null);
    }

    @Test
    public void booleanTrueTest() throws Exception, SyntaxErrorException {
    ...
    @Test
    public void booleanFalseTest() throws Exception, SyntaxErrorException {
        Compiler compiler=new Compiler(new StringReader("{isOdd isEven ->\n" +
                "\tisOdd = {(n) -> if n == 0 then false else isEven(n - 1) fi};\n" +
                "\tisEven = {(n) -> if n == 0 then true else isOdd(n - 1) fi};\n" +
                "\n" +
    ...
    @Test
    public void trueFalseTest() throws Exception, SyntaxErrorException {
        Compiler compiler=new Compiler(new StringReader("{isOdd isEven ->\n" +
                "\tisOdd = {(n) -> if n == 0 then false else isEven(n - 1) fi};\n" +
                "\tisEven = {(n) -> if n == 0 then true else isOdd(n - 1) fi};\n" +
                "\n" +
    ...
    public void stringTest() throws Exception, SyntaxErrorException {
        Compiler compiler=new Compiler(new StringReader("{ ->\n" +
                "\tprint(\"Hello, world!\\n\");\n" +
                "\tprint(\"Hi!\", \"\\n\");\n" +
                "\tprintln(\"你好\");\n" +
                "}"));
        assertEquals("你好", new InvokeExpr(compiler.program(), new ArrayList<>()).eval(null).toString());
    }

    @Test
    public void averageTest() throws Exception, SyntaxErrorException {
        Compiler compiler=new Compiler(new StringReader("    {average sqr abs sqrt x ->\n" +
                "            average = {(x y) -> (x + y) / 2};\n" +
                "        sqr = {(x) -> x * x};\n" +
                ...
                "    }"));
        assertEquals("true", new InvokeExpr(compiler.program(), new ArrayList<>()).eval(null).toString());
    }


    @Test
    public void numTest() throws Exception, SyntaxErrorException {
        Compiler c = new Compiler(new StringReader("{a sqr x ->\n" +
                ...
                "    println(3.27 % .7);\n" +
                "}"));
        new InvokeExpr(c.program(),new ArrayList<>()).eval(null).toString();
    }
